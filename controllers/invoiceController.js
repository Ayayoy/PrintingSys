...
const generateInvoice = async (orderId, totalCost, paymentCode, deliveryTime, next) => { 
    try {
        const invoice = new Invoice({ order: orderId, totalCost, paymentCode, deliveryTime });
        await invoice.save();
        return invoice;
    } catch (error) {
        next(error);
...
const getInvoiceByOrderId = async (req, res, next) => {
    try {
        ...
        if (!invoice) return res.status(200).json({ message: 'No invoice found for the order.' });
